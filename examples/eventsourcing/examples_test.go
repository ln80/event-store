...
import (
	...
	"github.com/ln80/event-store/examples/eventsourcing/bank"
...
func Example() {
	...
	if err := store.AppendToStream(ctx, sourcing.Wrap(ctx, stmID, event.VersionZero, acc.EvClearChanges())); err != nil {
		log.Fatal(err)
	}
	...
	if err := acc.WithdrawMoney(500); err != nil {
		log.Fatal(err)
	}
	if err := store.AppendToStream(ctx, sourcing.Wrap(ctx, stmID, stm.Version(), acc.EvClearChanges())); err != nil {
		log.Fatal(err)
	}
	...
	// Output:
	// stream version: 00000000000000000001.001e
	// evt 1: bank.AccountOpened, ver: 00000000000000000001.000~
	// evt 2: bank.MoneyDeposited, ver: 00000000000000000001.001e
	// account balance: 1000
	// stream version: 00000000000000000002.000e
	// evt 1: bank.MoneyWithdrawn, ver: 00000000000000000002.000e
	// account balance: 500
}
