...
func TestConvertEvent(t *testing.T) {
	...
	t.Run("convert enriched envelope to json Event", func(t *testing.T) {
		...
		evt := event.Wrap(ctx, stmID, testutil.GenEvents(1), func(env event.RWEnvelope) {
			env.SetUser(user)
			env.SetVersion(ver)
...
func TestMarshalUnmarshal(t *testing.T) {
	...
	// Notes:
	// first, test marshal/unmarshal non-registered events;
	// at first place envelope will be lazily unmarshaled; the envelope Event() method will return a nil event;
	// even though the domain event raw data must be preserved;
	// marshaling the unmarshaled result must be the same as the original envelope;
	// the envelope Event() will return the domain event once events are registered.
	...
	t.Run("marshal/unmarshal batch case", func(t *testing.T) {
		namespace := "service2"
		ser := NewEventSerializer(namespace)
		events := event.Wrap(ctx, stmID, testutil.GenEvents(4))
		b, _, err := ser.MarshalEventBatch(events)
		if err != nil {
			t.Fatalf("expect err be nil, got %v", err)
		}
		revents, err := ser.UnmarshalEventBatch(b)
		if err != nil {
			t.Fatalf("expect err be nil, got %v", err)
		}

		if wantl, l := len(events), len(revents); wantl != l {
			t.Fatalf("expect events len %d, %d be equals", wantl, l)
		}
		for i, revt := range revents {
			assertCmpEvt(t, revt, events[i], true)
		}

		// make sure we do not lose data even if we marshal x2
		b2, _, err := ser.MarshalEventBatch(revents)
		if err != nil {
			t.Fatalf("expect err be nil, got %v", err)
		}
		...
		// register events, so serializer can unmarshal event data
		testutil.RegisterEvent(namespace)
		revents, err = ser.UnmarshalEventBatch(b2)
		if err != nil {
			t.Fatalf("expect err be nil, got %v", err)
		}
		for i, revt := range revents {
			assertCmpEvt(t, revt, events[i], false)
		}
	})
}
